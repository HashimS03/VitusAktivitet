...
app.post("/user-history", authenticateJWT, async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ success: true, message: "User history saved successfully" });
  } catch (err) {
    serverLog("error", "User history save error:", err);
    res.status(500).json({
      success: false,
      message: `Failed to save user history: ${err.message}`,
    });
  }
});
...
app.get("/user-history", authenticateJWT, async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: `Failed to fetch user history: ${err.message}`,
    });
  }
});
